...
Camera camera;
Model carroceria, llanta, cofre;

Skybox skybox;

Sphere cabeza = Sphere(0.25, 20, 20);
...
int main()
{
	...
	{
		cabeza.init(); //inicializar esfera
		cabeza.load();//enviar la esfera al sh
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		carroceria.RenderModel();//modificar por el modelo sin las 4 patas y sin cola
